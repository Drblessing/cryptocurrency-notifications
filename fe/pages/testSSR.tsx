...
const sampleGainers = `name,price,change,href_id,api_id,symbol,contract_address
...
const parseToken = (token: string[]): GainerToken => {
  ...
  const percent_change_formatted = percent_change
    ...
    .replace('"', '')
    .replace(',', '');
  const percent_change_formatted_number = parseFloat(percent_change_formatted);
 ...
 */
export const parseGainers = (gainers: string): GainerToken[] => {
...
export const getServerSideProps: GetServerSideProps = async (context) => {
  // Handle local development since R2 is not available
  if (process.env.NODE_ENV === 'development') {
    const gainers = parseGainers(sampleGainers);
    return {
      props: {
        gainers,
      },
...
export default function TestSSR({ gainers, results }: DataProps) {
  // Display gainers
  // console.log(gainers);
  // Turn array of objects into a string
  const gainersString = JSON.stringify(gainers);
  return (
    <div>
      <h1>Test SSR</h1>
      <DataTable columns={columns} data={gainers} />
    </div>
  );
}
